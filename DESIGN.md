...
- **Oturum zarfı (D-2):** Her koltuk çağrısı, kod tarafından kurulan sabit bir zarfla başlar: BD'nin F0 fikir özeti, seçilen HMW, Denetçi'nin çerçeve itirazı, Şah'ın KAPI 2 metni, ek belge özeti. Zarf KAPI 2'den sonra DONAR; onu yalnız re-table değiştirir. Gerekçe: "bütün müzakere seçilen çerçevede yürür" cümlesi ancak çerçeve her çağrıya gerçekten giderse doğrudur; bir prompt satırıyla değil, yapıyla sağlanır. F0'ın HMW turu yalnız fikir özetini, F1 özet + HMW'yi, F2 ve sonrası zarfın tamamını görür.
...
- **Katmanlı prompt mimarisi (D-1):** Her koltuk çağrısı dört katmandan kurulur ve sıra sabittir: (1) **KİMLİK**, koltuk başına sabit ve bütün fazlarda aynı (`prompts/<koltuk>-kimlik.md`: kimsin, şapkan, neyi asla yapmazsın, kurulda neden varsın); (2) **OTURUM ZARFI** (§5 D-2); (3) **FAZ TALİMATI** (`prompts/<koltuk>-<faz>.md`); (4) **DİNAMİK BAĞLAM** (önceki faz özeti, sana yöneltilmiş itirazlar, tur bilgisi). Sistem mesajı kimlik + faz talimatından, kullanıcı mesajı zarf + ek (tam metin ya da özet) + dinamik bağlamdan oluşur. Sabit katmanlar başta durur ki sağlayıcı önbelleği işleyebilsin; etkisi tahmin edilmez, çağrı kaydındaki `cachedTokens` ile ÖLÇÜLÜR. **M2-C'de (onaylandı 2026-09-11):** Anthropic modellerinde sabit katmanlar `cache_control` ile işaretlenir: sistem bloğu ve zarf + fikir + ekler bloğu (istek başına en çok dört işaret; yazma 1.25x, okuma 0.1x, ömür 5 dk). **Asgari önbelleklenebilir ön ek modele bağlıdır ve iki kaynak farklı söylüyor** (OpenRouter: Opus 4.8 için 4.096 token; Anthropic: Opus 4.8 ve Sonnet 5 için 1.024); değer M2-C-1 probuyla ÖLÇÜLÜR. Doğrulanmamış bir sayı tasarıma girmez. Etkisi `cachedTokens` ve `cacheWriteTokens` ile ölçülür. Gerekçe: Divan'da ajan seansı yoktur, her çağrı hafızasızdır; bir insanın kurula girerken aldığı brifingin karşılığı, her çağrıda yeniden gönderilen bu sabit katmandır.
