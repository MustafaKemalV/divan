...
- **Katmanlı prompt mimarisi (D-1):** Her koltuk çağrısı dört katmandan kurulur ve sıra sabittir: (1) **KİMLİK**, koltuk başına sabit ve bütün fazlarda aynı (`prompts/<koltuk>-kimlik.md`: kimsin, şapkan, neyi asla yapmazsın, kurulda neden varsın); (2) **OTURUM ZARFI** (§5 D-2); (3) **FAZ TALİMATI** (`prompts/<koltuk>-<faz>.md`); (4) **DİNAMİK BAĞLAM** (önceki faz özeti, sana yöneltilmiş itirazlar, tur bilgisi). Sistem mesajı kimlik + faz talimatından, kullanıcı mesajı zarf + ek (tam metin ya da özet) + dinamik bağlamdan oluşur. Sabit katmanlar başta durur ki sağlayıcı önbelleği işleyebilsin; etkisi tahmin edilmez, çağrı kaydındaki `cachedTokens` ile ÖLÇÜLÜR. Gerekçe: Divan'da ajan seansı yoktur, her çağrı hafızasızdır; bir insanın kurula girerken aldığı brifingin karşılığı, her çağrıda yeniden gönderilen bu sabit katmandır.
