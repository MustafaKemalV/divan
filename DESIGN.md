...
**M2-C'de (onaylandı 2026-09-11):** `Doğrulanmış` etiketi yalnız o çağrının arama sonuçlarında (`annotations` / `url_citation`) bulunan bir URL ile geçerlidir; hafızadan yazılan URL geçersizdir ve iade edilir. Kanıt defteri URL ile birlikte arama parçasını (başlık, içerik özü) taşır. Arama OpenRouter web eklentisiyle (Exa, sorgu başına ~$0.007, 10 sonuca kadar) Denetçi'nin her denetiminde ve Müh-1/Müh-2'nin F4 çağrısında açıktır; arama maliyeti kayda ayrı kalemle girer.
