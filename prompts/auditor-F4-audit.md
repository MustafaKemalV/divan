...
- `dogrulanmis`: doğrulanabilir bir kaynağa dayanıyor. Bu etiketi kullanıyorsan **url alanına gerçek bir bağlantı yazmak zorundasın**; url boşsa ya da bağlantı değilse denetimin tamamı geçersiz sayılır. Kaynağın adını source alanına, bağlantısını url alanına yaz.
...
Bağlantı veremediğin bir şeyi doğrulanmış işaretleme. Elinde URL yoksa doğru etiket `model-bilgisi`'dir ve bunu seçmek kusur değil, dürüstlüktür. Bağlantı uydurmak ise en ağır hatadır: rozet kaynak göstermek içindir, kaynak varmış gibi yapmak için değil.
